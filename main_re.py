...
def main(args):
    ...
    for fold_idx in [3]: 
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            torch.save(model, f'./checkpoints/{args.dataset}_{args.gnn}_{args.read_op}_{fold_idx}.pth')
                
            if no_improve_cnt > args.early_stop:
                break
            elif test_perf['acc'] > best_valid_perf:
                best_valid_perf = test_perf['acc']
                no_improve_cnt = 0
                if args.dataset == 'DD':
                    # if test_perf['acc']>75.5 and test_perf['acc']<78:
                        torch.save(model, f'./checkpoints/{args.dataset}_{args.gnn}_{args.read_op}_{fold_idx}.pth')
                
                if args.dataset == 'MUTAG':
                    if args.gnn == 'gat':
                        if test_perf['acc']>76 and test_perf['acc']<82:
                            torch.save(model, f'./checkpoints/{args.dataset}_{args.gnn}_{args.read_op}_{fold_idx}.pth')
                    if args.gnn == 'gcn':
                        if test_perf['acc']>85 and test_perf['acc']<92:
                            torch.save(model, f'./checkpoints/{args.dataset}_{args.gnn}_{args.read_op}_{fold_idx}.pth')
                ...
                if args.dataset == 'Mutagenicity':
                    if args.gnn == 'gat':
                        if test_perf['acc']>77 and test_perf['acc']<80.5:
                            torch.save(model, f'./checkpoints/{args.dataset}_{args.gnn}_{args.read_op}_{fold_idx}.pth')
                    if args.gnn == 'gcn':
                        if test_perf['acc']>81 and test_perf['acc']<83:
                            torch.save(model, f'./checkpoints/{args.dataset}_{args.gnn}_{args.read_op}_{fold_idx}.pth')
