...
def random_permute(X):
    ...
    """
    X = X.transpose(1, 2)
    b = torch.rand((X.size(0), X.size(1))).to(args.device)
    idx = b.sort(0)[1]
...
def adj_loss_(l_enc, g_enc, edge_index, batch):
    ...
    adj = torch.zeros((num_nodes, num_nodes)).to(args.device)
    mask = torch.eye(num_nodes).to(args.device)
...
class InfoGraph(nn.Module):
  def __init__(self, read_op, dataset_num_features, hidden_dim, num_gc_layers, alpha=0.5, beta=1., gamma=.1):
    ...
    if read_op == 'set2set': self.embedding_dim  = hidden_dim * num_gc_layers * 2
    if read_op == 'sread': self.embedding_dim  = hidden_dim * num_gc_layers * 4
    self.local_d = FF(hidden_dim * num_gc_layers, hidden_dim * num_gc_layers)
    self.global_d = FF(self.embedding_dim, hidden_dim * num_gc_layers)
...
def clustering(Cluster, feature, true_labels, cluster = "kmeans", epoch = 0):
    tsne(feature, true_labels, f'{args.read_op}_{args.dataset}_{epoch}')
    feature = normalize(feature)
    if cluster == "kmeans":
        f_adj = feature
    else:
        f_adj = feature@feature.T
...
def main(args):
    ...
    print('================')
    print(device)
    model = InfoGraph(args.read_op, dataset_num_features, args.emb_dim, args.num_layer).to(device)
    ...
    for epoch in range(1, args.max_epochs+1):
        ...
        for data in dataloader:
            data = data.to(device)
            optimizer.zero_grad()
            loss = model(data.x, data.edge_index, data.batch, data.num_graphs, data)
        ...
        if epoch % 10 == 0:
            model.eval()
            emb, y = model.encoder.get_embeddings(dataloader)
            acc, nmi, ari = clustering(Cluster, emb, y, cluster='kmeans', epoch=epoch)
